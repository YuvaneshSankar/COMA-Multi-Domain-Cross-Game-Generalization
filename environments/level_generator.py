    ...
    def __init__(
        ...
        **env_kwargs
        ...
        Args:
            ...
            base_env: An instance of the base environment (preferred)
            base_env_class: ProceduralDungeonEnv class (optional if base_env provided)
            difficulty: Difficulty level for generation
            env_kwargs: Additional arguments for base environment (used if base_env_class provided)
        """
        self.generator = generator
        self.difficulty = difficulty
        self.env_kwargs = env_kwargs

        # Allow passing either an environment instance (base_env) or a class (base_env_class)
        if base_env is not None:
            self.env = base_env
        elif base_env_class is not None:
            self.env = base_env_class(**env_kwargs)
        else:
            raise ValueError("GeneratedDungeonEnv requires either base_env (instance) or base_env_class (class)")
